...
public class AES {
    ...
    private static final String keyBytes = "382d2402cd6d7191";
    ...
    public static String encode2(String content) {
        //加密之后的字节数组,转成16进制的字符串形式输出
        return parseByte2HexStr(encrypt(content, "abcdefgabcdefg12"));
    }

    public static class AESEncodeException extends Exception {
        public AESEncodeException(Throwable t){
            super(t);
        }
    }

    public static class AESDecodeException extends Exception {
        public AESDecodeException(Throwable t){
            super(t);
        }
     ...
     * 解密
     */
    public static String decode(String content) throws AESDecodeException{
        try {
            //解密之前,先将输入的字符串按照16进制转成二进制的字节数组,作为待解密的内容输入
            byte[] b = decrypt(parseHexStr2Byte(content), keyBytes);
            return new String(b);
        } catch (Exception e){
            throw new AESDecodeException(e);
        }
    }
